...
class Prices(commands.Cog, command_attrs=dict(hidden=True)):
    ...
    def __init__(self, bot):
        ...
        query = self.db.query(prices.PriceATH).filter(
            prices.PriceATH.token == self.bot.token['contract'])
    ...
    async def update_price(self):
        ...
        if self.current_ath:
            ...
            ath = prices.PriceATH(
                token=self.bot.token['contract'], price=self.bot.current_price)
            self.db.add(ath)
            self.db.commit()
    ...
    async def lp(self, ctx: commands.Context, num_tokens=None):
        ...
        with ctx.typing():
            ...
            output_header = f"{format(num_tokens, '.12g')} {self.bot.common['name']}/BNB LP"
            output_body = f"{self.bot.icon_value(token_value)} + {bnb_emoji} {bnb_value}"

        embed = discord.Embed(
            color=0x98FB98, title=output_header, description=output_body)
    ...
    async def convert(self, ctx: commands.Context, num_tokens=None):
        ...
        output_header = f"{format(float(num_tokens.quantize(self.bot.display_precision)), '.12g')} {self.bot.common['name']}"
        output_body = f"{bnb_emoji} **{Decimal(token_in_bnb * num_tokens).quantize(self.bot.display_precision)}**"

        usd = Decimal(token_in_bnb * self.bot.bnb_price *
                      num_tokens).quantize(self.bot.display_precision)
    ...
    async def ath(self, ctx: commands.Context):
        ...
        output_body = f"The highest price for {self.bot.common['name']} was **${self.current_ath.price}**"

        embed = discord.Embed(color=discord.Color.green(
        ), title=token_emoji + ' All Time High', description=output_body)

        amm_info = self.bot.get_amm()
        if amm_info.get('name'):
            embed.set_footer(
                text=f"Recorded {time} via {amm_info.get('name')}")
    ...
    async def round(self, ctx: commands.Context, address):
        ...
        async with ctx.typing():
            ...
            token_balance = Decimal(
                self.bot.contracts['token'].functions.balanceOf(address).call()) / exponent
            ...
            if token_balance:
                ...
                while whole_number > (mag := (10 ** i)):
                    ...
                    lines.append([str(target_balance), str(
                        target_balance - token_balance)])
            else:
                embed = discord.Embed(color=0xff2400, title="No Balance",
                                      description=f"This wallet has 0 {self.bot.token['icon']}")
                return await ctx.message.reply(embed=embed)

        col_widths = [max(len(s[0]) for s in lines),
                      max(len(s[1]) for s in lines)]
        header = lines.pop(0)
        ...
        ), title="Balance Rounding", description=output)
